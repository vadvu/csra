...
The **covariance** of random vector $X=[x_1,...,x_i]^T$ is $V(X)$:
$$V(X)=\begin{bmatrix}
...
Minimization of that sum (the quadratic amount of deviations between modeled and observable values) can be done by taking *partial derivatives* with respect to $\beta_k$ (lets write them more concise as $\theta=[\beta_1, \beta_2, ... \beta_p]$) because they are coefficients of linear combination of $X$ (matrix of explanatory variables). Such a matrix has following view: $$\begin{bmatrix}
    ...
    x_{i1} & \dots & x_{ij} \\
\end{bmatrix}$$ 
where columns $j$ are some variables and raws $i$ are some units. For example, units are countries (Russia, USA, China) and variables are their population and democracy level. 
...
Error term structure: $e_{i,t} = e_i + e_t$ (*cross-sectional* variance + *time dimension* variance).   
...
## Logistic regression 
Logistic regression models binary dependent variable using *Logit link*. Moreover, it does it in terms of probabilities. Logistic function is:
...
exposing the matrix is an extremely difficult task 
...
One such method is logistic regression with a special estimator (Firth, 1993; Kosmidis et al., 2020). 

One more approach involves deliberately removing a significant number of '0's to balance the number of observations in both classes (known as under-sampling, discussed in Menardi and Torelli, 2014). This approach enables the use of classical logistic regression without the concern of imbalance.
...
## Logistic regression with specific estimator 


We will use data from `csra` - `dataex` to model association between regime type (VDEM electoral democracy index) and nonviolent (unarmed) revolutions using NAVCO 1.3 database. Note, we have to use regime type with lag (at $t-1$ time) to avoid endogeneity due to indeterminate association ($Y \to X$), because the goal of majority revolutions to change regime type. 
...
datex <- datex %>% drop_na() %>% mutate(NVC_1.3_NONVIOL = as.numeric(NVC_1.3_NONVIOL)-1)
```


```{r}
logistic.log <- glm(data = datex, 
                NVC_1.3_NONVIOL ~ VDEM_v2x_polyarchy_lag + I(VDEM_v2x_polyarchy_lag^2), 
                family = binomial(link = "logit"))

logistic.clog <- glm(data = datex, 
                NVC_1.3_NONVIOL ~ VDEM_v2x_polyarchy_lag + I(VDEM_v2x_polyarchy_lag^2), 
                family = binomial(link = "cloglog"))
```

```{r, results='asis', echo=FALSE}
texreg::htmlreg(list(logistic.log, logistic.clog), custom.model.names = c("logit", "cloglog"))
```
...
library(Zelig)
tau = sum(datex$NVC_1.3_NONVIOL==1)/nrow(datex)

relogit.pr = zelig(data = datex,
                NVC_1.3_NONVIOL ~ VDEM_v2x_polyarchy_lag + I(VDEM_v2x_polyarchy_lag^2),
                model = "relogit",
                tau = tau,
                case.control = "prior",
                bias.correct = TRUE, cite = F)
```


```{r, results='asis', echo=FALSE}

texreg::htmlreg(list(logistic.log, relogit.pr), 
                custom.model.names = c("logit", "relogit.prior"))

```


a problem with standard errors 

```{r, results='asis', echo=FALSE}

# texreg::htmlreg(list(logistic.log, relogit.pr, relogit.pr.se), custom.model.names = c("logit", "relogit"))
...
comp_pred <- function(x){
  datn <- data.frame(VDEM_v2x_polyarchy_lag = seq(0,1,0.01)) %>% 
    mutate(`I(VDEM_v2x_polyarchy_lag^2)` = VDEM_v2x_polyarchy_lag^2)
  for (model in x){
    i = i+1
    pred <- predict(model, newdata = datn, se.fit = T)
    datn <- datn %>% mutate(k = pred$fit, kk = pred$se.fit)
    
  }
}
x <- list(logistic.log = logistic.log, logist = logist)
names(x)
```
...
brglm.jeff = glm(data = datex,
                NVC_1.3_NONVIOL ~ VDEM_v2x_polyarchy_lag + I(VDEM_v2x_polyarchy_lag^2),
                family = binomial(link = "logit"),
                method = "brglmFit", type = "MPL_Jeffreys")

brglm.mean = glm(data = datex,
                NVC_1.3_NONVIOL ~ VDEM_v2x_polyarchy_lag + I(VDEM_v2x_polyarchy_lag^2),
                family = binomial(link = "logit"),
...
texreg::htmlreg(list(logistic.log, brglm.jeff, brglm.mean), custom.model.names = c("logit", "MPL_Jeffreys", "AS_median"))
